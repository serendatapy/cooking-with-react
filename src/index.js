...
3.1 RecipeList -
Takes all recipes form App, and then passes them destructured one by one
to Recipe in JSX format, assigning a key (necessary when using arrays).
Thanks to context is able to access handleRecipeAdd function from App.
...
3.3 IngredientList - IngredientList further breaks up the ingredients object
  passing each one to the Ingredient component

3.4 Ingredient - each ingredient is layed out as indicated, wrapped in JSX
when returned back up

4. RecipeEdit - displays all fields to allow editing, also connects
to recipeIngredientEdit
*takes 'recipe' as a prop from App, and takes values from there to populate
fields.
